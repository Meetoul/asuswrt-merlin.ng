...
export RT-AX58U_V2 += BUILD_NAME="RT-AX58U_V2" NVSIZE="128" DHDAP=n HND_WL=y DPSTA=n LACP=n WTFAST=n REPEATER=y IPV6SUPP=y \
		HTTPS=y ARM=y AUTODICT=y BBEXTRAS=y USBEXTRAS=y EBTABLES=y MEDIASRV=y MODEM=y USB_WAN_BACKUP=y PARENTAL2=y ACCEL_PPTPD=y PRINTER=y WEBDAV=y \
		SMARTSYNCBASE=y USB="USB" APP="installed" PROXYSTA=y DNSMQ=y SHP=n BCMWL6=y BCMWL6A=y DISK_MONITOR=y BTN_WIFITOG=n \
		OPTIMIZE_XBOX=y ODMPID=y LED_BTN=n BCMSMP=y XHCI=y DUALWAN=y NEW_USER_LOW_RSSI=y OPENVPN=y TIMEMACHINE=y MDNS=y VPNC=y \
		JFFS2LOG=y NOTIFICATION_CENTER=y BWDPI=y DUMP_OOPS_MSG=n LINUX_MTD="64" DEBUGFS=y TEMPROOTFS=n SSH=y EMAIL=y FRS_FEEDBACK=y \
		...
		ASPMD=n BCM_MEVENT=n BCMEVENTD=y BCM_APPEVENTD=n LETSENCRYPT=y JFFS_NVRAM=y JFFS_NVRAM_HND_OLD=y NVRAM_ENCRYPT=y \
		IPSEC=STRONGSWAN IPSEC_SRVCLI_ONLY=SRV NATNL_AIHOME=y BCM_CEVENTD=y UTF8_SSID=y AMAS=y DWB=y DBLOG=y ETHOBD=y CFGSYNC_LOCSYNC=y \
		CONNDIAG=y CRASHLOG=y WATCH_REINIT=n BW160M=y BRCM_HOSTAPD=y UUPLUGIN=n IPERF3=y FRS_LIVE_UPDATE=y ASUSCTRL=y BCN_RPT=y \
		BTM_11V=y INFO_EXAP=y AVBLCHAN=y SW_CTRL_ALLLED=y ASD=y AHS=y INTERNETCTRL=y GAME_MODE=n INSTANT_GUARD=y ACL96=y GOOGLE_ASST=y \
		COMFW=y AMAS_ETHDETECT=y IPV6S46=y NFS=Y WIREGUARD=y
